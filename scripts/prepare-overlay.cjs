...
function install(name,source,entries,version){const [author,project]=name.split("/"),dest=path.join(packages,author,project,version);fs.mkdirSync(dest,{recursive:true});for(const entry of entries)fs.cpSync(path.join(source,entry),path.join(dest,entry),{recursive:true});}
const pkg=JSON.parse(fs.readFileSync(path.join(root,"elm.json"),"utf8"));
function versionBytes(v){const p=v.split(".").map(Number);if(p.length!==3||p.some(n=>!Number.isInteger(n)||n<0||n>254))throw new Error("unsupported package version "+v);return Buffer.from(p);}
install("sjalq/schelm-node-sqlite",root,["src","elm.json","README.md","LICENSE"],pkg.version);
...
function add(buf,author,project,version){const {n,xs}=entries(buf),key=`${author}/${project}`;if(xs.some(x=>`${x.author}/${x.project}`===key))return buf;const index=xs.findIndex(x=>`${x.author}/${x.project}`>key),at=index<0?buf.length:xs[index].start;const entry=Buffer.concat([Buffer.from([Buffer.byteLength(author)]),Buffer.from(author),Buffer.from([Buffer.byteLength(project)]),Buffer.from(project),versionBytes(version),Buffer.alloc(8)]),out=Buffer.concat([buf.subarray(0,at),entry,buf.subarray(at)]);out.writeBigUInt64BE(buf.readBigUInt64BE(0)+1n,0);out.writeBigUInt64BE(BigInt(n+1),8);return out;}
registry=add(registry,"sjalq","schelm-node-sqlite",pkg.version);fs.writeFileSync(path.join(packages,"registry.dat"),registry);
