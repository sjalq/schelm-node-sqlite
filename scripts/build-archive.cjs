...
const tarPath=out.replace(/\.gz$/,'');cp.execFileSync('tar',['--sort=name','--mtime=UTC 2020-01-01','--owner=0','--group=0','--numeric-owner','-cf',tarPath,'-T',list],{cwd:root});const fd=fs.openSync(out,'w');try{cp.execFileSync('gzip',['-n','-c',tarPath],{stdio:['ignore',fd,'inherit']});}finally{fs.closeSync(fd);}fs.rmSync(tarPath);
