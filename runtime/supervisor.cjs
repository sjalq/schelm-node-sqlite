...
function toParent(m){if(!process.connected)return;process.send(m,()=>{if(m.kind==='row'||m.kind==='columns')parentCredit=1;else{pending=null;parentCredit=1;}});}
function fromWorker(m){if(m.kind==='ready'&&m.v===VERSION){ready=true;process.send({v:VERSION,id:0,kind:'ready',creditMessages:1,creditBytes:MAX_FRAME});return;}if(!pending||!m||m.v!==VERSION||m.id!==pending.id)throw new Error('worker identity');if(m.kind==='columns'||m.kind==='row'||m.kind==='done'||m.kind==='error'){toParent(m);return;}throw new Error('worker tag');}
...
function boot(){worker=spawn(process.execPath,['--max-old-space-size=128','-e',workerSource],{stdio:['pipe','pipe','ignore'],detached:true});worker.stdout.on('data',c=>{try{consume('worker',c,fromWorker);}catch(_){kill('SIGKILL');}});worker.on('exit',(code,signal)=>{const p=pending;pending=null;ready=false;if(p&&process.connected)process.send({v:VERSION,id:p.id,kind:'exit',code:code||0,signal:signal||''},()=>process.exit(70));else process.exit(killing?0:70);});}
process.on('message',m=>{try{fromParent(m);}catch(_){kill('SIGKILL');process.exit(71);}});
const meter=setInterval(()=>{if(worker&&rssKb(worker.pid)>HARD_RSS_KB){const p=pending;killing=true;kill('SIGKILL');if(p&&process.connected)process.send({v:VERSION,id:p.id,kind:'memory'});}},25);meter.unref();
